    ...
    if let Some(thinking) = &claude_req.thinking {
        ...
        if thinking.type_ == "enabled" && is_thinking_enabled {
            ...
            if let Some(budget_tokens) = thinking.budget_tokens {
                ...
                // [FIX] Enforce minimum 1024 (API requirement)
                // Error: "thinking.enabled.budget_tokens: Input should be greater than or equal to 1024"
                ...
                tracing::debug!(
                    "[Generation-Config] thinkingBudget: {} (original: {}, flash_capped: {})",
                    budget,
                    budget_tokens,
                    is_flash_model
                );
